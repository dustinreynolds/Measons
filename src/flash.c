...
uint8_t flash_stress_test(void){
	...
	uint32_t page = 0;
#ifdef DEBUG_FLASH
	uart_OutString(USART2,"SPI Flash Erase All: ");
#endif
	timeout = flash_erase_all();
	if(timeout == 0){
#ifdef DEBUG_FLASH
		while(1){
			uart_OutString(USART2,"Erase All Timed Out\r\n");
		}
#else
		return 0;//failed
#endif
...
#ifdef DEBUG_FLASH
		uart_OutString(USART2,"Erase All Successful\r\n");
#endif
	}

	flash_view_status();
#ifdef DEBUG_FLASH
	uart_OutString(USART2,"Flash Stress Test In Progress\r\n");
#endif
